// 付費插件協定:registry 寫的文字(402 的 message、條目名稱)一律先經過這裡才印出。
// 惡意或寫壞的 registry 頂多在自己的條目上寫錯字:不能在終端機裡送控制碼(清畫面、改
// 視窗標題、蓋掉前一行輸出),也不能用雙向文字控制字元把字排成另一個意思。
//
// 與 core 的 src/lib/registry-text.ts 是同一份實作(這支 CLI 不 import core,見 registry.ts
// 檔頭)。core 的 test/registry-text.test.ts 拿同一組輸入比對兩邊的輸出 —— 改一邊就得改另一邊。
