...
//
// Alt text: the card chrome (white surface, shadows, selection ring) moved onto
// a wrapping <div> so the alt editor can live INSIDE the card but OUTSIDE the
// selection <button> — a button inside a button is invalid HTML and the nested
// input would toggle selection on every click. Only images get the row; alt is
// meaningless for a PDF or a zip.

interface MediaGridProps {
  files: StoredFileDTO[];
  selected: ReadonlySet<string>;
  onToggle: (key: string) => void;
  /** Resolves true when the alt was persisted. */
  onAltSave: (key: string, alt: string) => Promise<boolean>;
}
...
function GridItem({
  ...
  selected: boolean;
  onToggle: () => void;
  onAltSave: (alt: string) => Promise<boolean>;
}) {
  ...
  return (
    ...
    >
      ...
      {img && (
        <MediaAltField key={file.alt ?? ""} alt={file.alt} onSave={onAltSave} />
      )}
    </div>
  );
}

export function MediaGrid({ files, selected, onToggle, onAltSave }: MediaGridProps) {
  return (
      ...
      {files.map((file) => (
        <li key={file.key} className="w-[11rem] max-w-full shrink-0">
          <GridItem
            ...
            onAltSave={(alt) => onAltSave(file.key, alt)}
          />
        </li>
      ))}
    </ul>
