...
// rounded-[calc(12px*var(--admin-radius-scale,1))] 容器、每列一個 ring-dot、tool 名 mono 兩段色、失敗紅字。
...
export function AuditLog({ rows, now, view, tool, empty }: AuditLogProps) {
  ...
  if (rows.length === 0) {
    return (
      <p className="rounded-[calc(12px*var(--admin-radius-scale,1))] bg-surface px-4 py-8 text-center text-[13px] text-ink/35 shadow-[var(--admin-shadow-card,0_0_0_1px_rgba(20,18,22,0.05),0_1px_2px_-1px_rgba(20,18,22,0.05))]">
        {empty}
      </p>
    );
  }
        ...
        return (
          <li key={row.id} className="flex flex-col">
            <button
              ...
              aria-expanded={open}
              className={cn(
                "group/row flex w-full items-center gap-2.5 rounded-[calc(8px*var(--admin-radius-scale,1))] px-2 py-2 text-left transition-colors duration-150 hover:bg-ink/[0.025]",
                open && "bg-ink/[0.025]",
              )}
            >
              <StatusDot tone={row.ok ? "good" : "draft"} className="shrink-0" />
              <span className="min-w-0 flex-1 truncate font-mono text-[11px] lowercase">
                <span className="text-ink/35">{toolNamespace(row.tool)}.</span>
                <span className="text-ink/75">{toolLeaf(row.tool)}</span>
              </span>
              <span
                className={cn(
                  "hidden shrink-0 rounded-[calc(5px*var(--admin-radius-scale,1))] px-1.5 py-px text-[10.5px] sm:inline",
                  row.kind === "write"
              ...
              <span className="hidden max-w-[11rem] shrink-0 truncate text-[11px] text-ink/40 md:inline">
                {row.userEmail}
              </span>
              <time
                dateTime={new Date(row.at).toISOString()}
                title={absoluteTime(row.at, dates)}
                className="shrink-0 text-[11px] tabular-nums text-ink/35"
              >
            ...
            {open && (
              <div className="flex flex-col gap-2.5 px-2 pt-1 pb-2.5">
                {/* 中繼資料一行:來源(自動 / 經確認)、誰(窄視窗時列頭放不下才在這裡補)、
                    ...
                    term/description 配對。 */}
                <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 text-[11px] text-ink/45">
                  <span
                    className={cn(
                  ...
                  {tool !== row.tool && (
                    <AdminLink
                      ...
                      className="ml-auto text-[11px] text-ink/40 underline-offset-2 transition-colors duration-150 hover:text-ink/75 hover:underline"
                    >
...
function Block({ label, body, danger }: { label: string; body: string; danger?: boolean }) {
  return (
    <div className="flex flex-col gap-1">
      <span className="font-mono text-[10.5px] lowercase text-ink/35">{label}</span>
      <pre
        className={cn(
          "max-h-56 overflow-auto rounded-[calc(8px*var(--admin-radius-scale,1))] px-2.5 py-2 font-mono text-[10.5px] leading-relaxed break-words whitespace-pre-wrap",
          danger ? "bg-red-50 text-red-700" : "bg-ink/[0.035] text-ink/55",
        )}
      >
        {body}
      </pre>
