...
export function useCyclePhase({ active, cycleMsBase, speed = 1 }: UseCyclePhaseOptions): number {
  ...
  // inactive 時歸零走 render-time adjust(effect body 同步 setState 會被
  // react-hooks/set-state-in-effect 擋)。
...
export function useSteppedCycle({
  ...
  // inactive 重置同 useCyclePhase:render-time adjust;ref 重置留在 effect(合法)。
  const [prevActive, setPrevActive] = useState(active);
  if (active !== prevActive) {
    setPrevActive(active);
