      ...
      {open && (
        ...
        >
          <motion.div
            ...
            // 同心圓:input rounded-[calc(10px*var(--admin-radius-scale,1))] + 外距 14px(p-3.5)→ 面板 24px。
            // (方向:收斂 input 的圓角,而不是把容器撐得更圓 —— 容器過圓會逼列
            // hover 變 pill、footer 貼弧線,整組跟著變形。)
            className="mx-auto mt-[12vh] flex max-h-[64vh] w-[min(40rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-[calc(24px*var(--admin-radius-scale,1))] bg-surface shadow-[var(--admin-shadow-panel,0_0_0_1px_rgba(0,0,0,0.06),0_24px_64px_-16px_rgba(30,20,50,0.25))]"
          >
            {/* 搜尋列:input 撐滿整寬,loader 與 close 都 inline 在最右端。 */}
            <div className="p-3.5 pb-2">
              <div className="flex h-11 items-center gap-2.5 rounded-[calc(10px*var(--admin-radius-scale,1))] bg-ink/[0.04] pr-1.5 pl-3.5 transition-shadow focus-within:shadow-[0_0_0_1.5px_rgba(0,0,0,0.14)]">
                ...
                <input
                  ...
                  className="h-full flex-1 bg-transparent text-[14px] text-ink/85 outline-none placeholder:text-ink/30"
                />
                ...
                <button
                  ...
                  className="flex size-8 shrink-0 items-center justify-center rounded-[calc(7px*var(--admin-radius-scale,1))] text-ink/35 transition-[background-color,color,transform] hover:bg-ink/[0.06] hover:text-ink/70 active:scale-[0.96]"
                >
                  <X className="size-4" />
                </button>
              </div>
              ...
              {showingRecent ? (
                recent.length > 0 ? (
                  <>
                    <div className="flex items-center justify-between px-4 pt-2 pb-1.5">
                      <p className="text-[11px] font-semibold tracking-[0.06em] text-ink/35 uppercase">
                        {t("search.recent")}
                      </p>
                      <button
                        type="button"
                        ...
                        className="text-[11.5px] text-ink/35 transition-colors hover:text-ink/60"
                      >
                        {t("search.clear")}
                      </button>
                    </div>
                    {recent.map((r, i) => (
                      <button
                        ...
                        className={cn(
                          "flex w-full items-center gap-3 rounded-[calc(10px*var(--admin-radius-scale,1))] px-3.5 py-2.5 text-left text-[13.5px] text-ink/70 transition-colors",
                          selected === i && "bg-ink/[0.04]",
                        )}
                      >
                        <Clock className="size-3.5 shrink-0 text-ink/30" />
                        <span className="truncate">{r}</span>
                      </button>
                    ))}
                  </>
                ) : (
                  <p className="px-3.5 py-8 text-center text-[13px] text-ink/35">
                    {t("search.hint")}
                  </p>
                )
              ) : hits.length > 0 ? (
                hits.map((hit, i) => (
                  <button
                    ...
                    onMouseMove={() => setSelected(i)}
                    className={cn(
                      "flex w-full items-center gap-3 rounded-[calc(10px*var(--admin-radius-scale,1))] px-3.5 py-2.5 text-left transition-colors",
                      selected === i && "bg-ink/[0.04]",
            ...
            <div className="flex items-center justify-between border-t border-ink/[0.06] px-5 py-3 text-[11.5px] text-ink/40">
...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="flex h-5 min-w-5 items-center justify-center rounded-[calc(5px*var(--admin-radius-scale,1))] bg-ink/[0.05] px-1.5 font-sans text-[10.5px] font-medium text-ink/50 shadow-[inset_0_-1px_0_rgba(0,0,0,0.06)]">
      {children}
    </kbd>
  );
}
