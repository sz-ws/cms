...
// 只能看這一頁的角色:兩區照列,沒有核可、退回、標記已收款。

const ORDERS_TABLE = "ext_shop_orders";

export async function ShopVerifyPage() {
  const [orders, pending, transferProvider, canEdit] = await Promise.all([
  ...
  const actionsEndpoint = canEdit ? "/api/ext/shop" : undefined;
    ...
    <div className="flex max-w-3xl flex-col gap-5">
      <header>
        ...
        <p className="mt-1 text-[13.5px] leading-relaxed text-black/55">
          客人回報匯款後在這裡對帳：以末五碼比對銀行入帳紀錄，核可即入帳（訂單標記已付款），對不到就退回（訂單回到待付款，客人可重新回報）。收款帳戶設定在
      ...
      <section>
        ...
        <TransferVerifyQueue orders={orders} actionsEndpoint={actionsEndpoint} />
      </section>
